...
- `zshenv`：universal shell（所有 zsh 进程）
  - 仅保留最小通用环境变量（如 locale）
- `zprofile`：login shell
  - 登录时初始化与 `PATH` 主权（如 `brew shellenv`）
- `zshrc`：interactive shell
  - 负责加载顺序与模块编排

## zshrc.d 下的职责

- `env.zsh`（Pre-OMZ）：交互环境变量（不维护 PATH）
- `plugins.zsh`（Pre-OMZ）：仅定义 `plugins=(...)`
- `aliases.zsh`（Post-OMZ）：别名
- `completion.zsh`（Post-OMZ）：补全行为与 fzf 集成
- `tools.zsh`（Post-OMZ）：工具初始化与 runtime 懒加载钩子（尽量懒加载）
- `local/*.zsh`（Post-OMZ）：本地或机器特定扩展

## Runtime 策略

- `mise` 作为主 runtime 管理器（通过 `~/.local/share/mise/shims`）
- `nvm` 作为按需 fallback，在首次执行 `nvm/node/npm/npx` 时懒加载
...
zshenv
放：所有 zsh 都需要的最基础环境（LANG、通用 locale）。
不放：alias、插件、prompt、任何慢命令（如大量 eval / 外部探测）。

zprofile
放：仅 login shell 需要的一次性初始化（例如 brew shellenv）与 PATH 组装。
...
env.zsh
放：交互 shell 专属环境变量。
不放：PATH 主权配置（现在已在 zprofile）。

tools.zsh
放：工具初始化与 hook（starship、mise 可选激活、thefuck 懒加载、nvm 懒加载）。
